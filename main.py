...
from fedalg import FedAvg, FedProx, FedAdp, FedImp, DyFedImp, FedAdagrad, FedAdam, FedYogi, FedAvgM

from model import fMLP, fCNN, eMLP, eCNN, CNN2, CNN4
import utils
...
client_resources = {"num_cpus": 2, "num_gpus": 0.2} if DEVICE.type == "cuda" else {"num_cpus": 1, "num_gpus": 0.0}

fl.simulation.start_simulation(
    client_fn = client_fn,
    num_clients = NUM_CLIENTS,
    config = fl.server.ServerConfig(num_rounds=NUM_ROUNDS),
    strategy = DyFedImp(num_rounds=NUM_ROUNDS, 
                        num_clients=NUM_CLIENTS,
                        iids = NUM_IIDS, 
